...
read -r user secret < <(echo $(kubectl -n hmpps-organisations-dev get secret hmpps-organisations-api-client-creds -o json | jq '.data[] |= @base64d' | jq -r '.data.SYSTEM_CLIENT_ID, .data.SYSTEM_CLIENT_SECRET'))
